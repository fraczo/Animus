...
namespace EventReceivers.admProcesy
{
    public class VAT_Forms
    {
        ...
        private static void Create_VAT_KW_Form(SPWeb web, int klientId, int okresId)
        {
            try
            {
                string key = BLL.tabZadania.Define_KEY(ctVAT, klientId, okresId);
                if (BLL.tabZadania.Check_KEY_IsAllowed(key, web, 0))
                {
                    ...
                    BLL.tabZadania.Create_ctVAT_Form(web, ctVAT, klientId, okresId, key, terminPlatnosci, terminPrzekazania, true);
                }
            }
            catch (Exception ex)
            {
        ...
        private static void Create_VAT_M_Form(SPWeb web, int klientId, int okresId)
        {
            ...
            try
            {
                ...
                if (BLL.tabZadania.Check_KEY_IsAllowed(key, web, 0))
                {
                    ...
                    BLL.tabZadania.Create_ctVAT_Form(web, ctVAT, klientId, okresId, key, terminPlatnosci, terminPrzekazania, false);
                }
            }
            catch (Exception ex)
            {
